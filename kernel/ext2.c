...
static int ext2_add_dirent(ext2_fs_t* fs, u32 dir_inode_num, const char* name, u32 child_inode, u8 file_type);
...
int ext2_mount(ata_disk_t* disk, u32 partition_start, ext2_fs_t** out_fs) {
    ...
    if (fs->sb.magic != EXT2_SUPER_MAGIC) {
        // format if magic is wrong
        if (ext2_format(disk, partition_start, 0) != 0) {
            kfree(fs);
            vga_write("failed to mount! ext2_format\n", COLOUR_DEBUG_ERROR);
            return -1;
        }
    ...
    if (!fs->bgdt) {
        kfree(fs);
        vga_write("failed to mount: bgdt allocation\n", COLOUR_DEBUG_ERROR);
        return -1;
    }
...
static int ext2_alloc_block(ext2_fs_t* fs) {
    (void)fs;
    static u32 next_block = 10; // simple incremental allocation
    return next_block++;
}
